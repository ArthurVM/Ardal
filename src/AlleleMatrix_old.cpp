 ...
 ****************************************************************************************************/
py::array_t<int> AlleleMatrix::hamming( bool fill_cache = false ) const {
...
/****************************************************************************************************
 * ardal::AlleleMatrix::hammingSIMD
 * 
 * Calculate the Hamming distances between all pairs of rows (SIMD optimized)..
 *
 * This function calculates the pairwise Hamming distances between all rows of the allele matrix.
 * The Hamming distance between two rows is the number of positions at which the corresponding
 * elements differ.  The results are returned as a condensed distance matrix.
 ...
 *                      where 'n' is the number of rows in the matrix.
 ****************************************************************************************************/
py::array_t<int> AlleleMatrix::hammingSIMD( bool fill_cache = false ) const {
    // access matrix (read only)
    auto matrix_acc = _matrix.unchecked<2>();
    ...
    for (size_t i = 0; i < n; ++i) {
        for (size_t j = i + 1; j < n; ++j) {  // iterate over rows
            ...
            if (cached_dist != _hamming_cache.end()) {
                distance = cached_dist->second;  // dist found and assigned
            } else {
                distance = 0;  // dist not found so initialised as 0
                // initialise accumulators and set to 0
                int d0 = 0, d1 = 0, d2 = 0, d3 = 0, d4 = 0, d5 = 0, d6 = 0, d7 = 0;

                // hamming distance calculation
                size_t l = 0;
                // SIMD part (AVX2) - process 32 alleles (bytes) at a time for binary data
                for (; l + 31 < _m; l += 32) {
                    // NOTE: assumes uint8_t. This assumption *should* be correct, but it will break spectacularly if it isnt
                    __m256i a = _mm256_loadu_si256((__m256i const*)matrix_acc.data(i, l));
                    __m256i b = _mm256_loadu_si256((__m256i const*)matrix_acc.data(j, l));
                    __m256i xor_result = _mm256_xor_si256(a, b); // Contains 32 bytes, each 0x00 or 0x01

                    // set bit count
                    d0 += _mm_popcnt_u32(_mm256_extract_epi32(xor_result, 0));
                    d1 += _mm_popcnt_u32(_mm256_extract_epi32(xor_result, 1));
                    d2 += _mm_popcnt_u32(_mm256_extract_epi32(xor_result, 2));
                    d3 += _mm_popcnt_u32(_mm256_extract_epi32(xor_result, 3));
                    d4 += _mm_popcnt_u32(_mm256_extract_epi32(xor_result, 4));
                    d5 += _mm_popcnt_u32(_mm256_extract_epi32(xor_result, 5));
                    d6 += _mm_popcnt_u32(_mm256_extract_epi32(xor_result, 6));
                    d7 += _mm_popcnt_u32(_mm256_extract_epi32(xor_result, 7));
                }
                // sum accumulators
                distance = d0 + d1 + d2 + d3 + d4 + d5 + d6 + d7;

                // remainder loop for alleles not fitting in 32-byte chunks
                for (; l < _m; ++l) {
                    if (matrix_acc(i, l) != matrix_acc(j, l)) {
                        distance++;
                    }
    ...
    for (size_t i = 0; i < _n; ++i) {
        if (i != row_coord) {
            ...
            if (cached_dist != _hamming_cache.end()) {
                ...
                distance = 0;
                // Use 8 separate 32-bit counters for instruction-level parallelism
                int d0 = 0, d1 = 0, d2 = 0, d3 = 0, d4 = 0, d5 = 0, d6 = 0, d7 = 0;
                // hamming distance using SIMD (AVX2)
                size_t j = 0;
                for (; j + 31 < _m; j += 32) {  // process in chunks of 32
                    // NOTE: assumes uint8_t. This assumption *should* be correct, but it will break spectacularly if it isnt
                    __m256i a = _mm256_loadu_si256((__m256i const*)matrix_acc.data(row_coord, j));    // load 32bytes from query row
                    __m256i b = _mm256_loadu_si256((__m256i const*)matrix_acc.data(i, j));            // load 32bytes from current row
 ...
 *
 * Find all GUIDs that share at least k SNPs with a given query GUID. Scalar implementation.
 *
...
py::list AlleleMatrix::findSharedSNPGUIDs( size_t row_coord, int k_snps ) const {
    ...
    py::list resulting_guids_list;
    
    // get query SNPs
    std::set<int> query_snps = accessGUID(static_cast<int>(row_coord));

    // if query GUID has fewer SNPs than k_snps, no matches are possible
    if (query_snps.size() < static_cast<size_t>(k_snps)) {
        return resulting_guids_list; // return empty list
    }
    ...
    for (size_t i = 0; i < _n; ++i) {
        ...
        // mass optimisation
        if (_rmass[i] < static_cast<int>(k_snps)) {
            continue;   // skip if the mass falls below the given number of shared snps
        }

        // get other GUID's SNPs
        std::set<int> other_snps = accessGUID(static_cast<int>(i));

        // if the other GUID has fewer SNPs than k_snps then skip
        if (other_snps.size() < static_cast<size_t>(k_snps)) {
            continue;
        }

        // compute intersection and get size
        std::set<int> intersection_set;
        std::set_intersection(query_snps.begin(), query_snps.end(),
                              other_snps.begin(), other_snps.end(),
                              std::inserter(intersection_set, intersection_set.begin()));
        size_t inner_product = intersection_set.size();

        if (inner_product >= k_snps) {
            resulting_guids_list.append(py::make_tuple(i, inner_product));   // append to the guid list 
        }
    }
    return resulting_guids_list;
}


/****************************************************************************************************
 * ardal::AlleleMatrix::findSharedSNPGUIDsSIMD
 *
 * Find all GUIDs that share at least k SNPs with a given query GUID with SIMD optimisation.
 *
 * This function identifies GUIDs (rows) in the allele matrix that have a specified minimum
 * number of SNPs in common with a target query GUID.
 *
 ...
 *   std::runtime_error : If query_guid_index is out of range or k_snps is negative.
 ****************************************************************************************************/
py::list AlleleMatrix::findSharedSNPGUIDsSIMD( size_t row_coord, int k_snps ) const {
    if (row_coord >= _n) {
        throw std::runtime_error("Query GUID index out of range.");
    }
    if (k_snps < 0) {
    ...
    // if query GUID has fewer SNPs than k_snps then no matches are possible
    if (static_cast<int>(_rmass[row_coord]) < k_snps) {
        return resulting_guids_list;
    }
    ...
    for (size_t i = 0; i < _n; ++i) {
        ...
        for (; l + 31 < _m; l += 32) {
            // NOTE: assumes uint8_t. This assumption *should* be correct, but it will break spectacularly if it isnt
            __m256i chunk_query = _mm256_loadu_si256((__m256i const*)(matrix_acc.data(row_coord, l)));
            __m256i chunk_other = _mm256_loadu_si256((__m256i const*)(matrix_acc.data(i, l)));
            __m256i and_result = _mm256_and_si256(chunk_query, chunk_other);

            // bit count stuff
            i0 += _mm_popcnt_u32(_mm256_extract_epi32(and_result, 0));
            i1 += _mm_popcnt_u32(_mm256_extract_epi32(and_result, 1));
            i2 += _mm_popcnt_u32(_mm256_extract_epi32(and_result, 2));
            i3 += _mm_popcnt_u32(_mm256_extract_epi32(and_result, 3));
            i4 += _mm_popcnt_u32(_mm256_extract_epi32(and_result, 4));
            i5 += _mm_popcnt_u32(_mm256_extract_epi32(and_result, 5));
            i6 += _mm_popcnt_u32(_mm256_extract_epi32(and_result, 6));
            i7 += _mm_popcnt_u32(_mm256_extract_epi32(and_result, 7));
        }
        // sum accumulators
        inner_product += i0 + i1 + i2 + i3 + i4 + i5 + i6 + i7;
        
        // remainder loop
        for (; l < _m; ++l) {
            if (matrix_acc(row_coord, l) == 1 && matrix_acc(i, l) == 1) {
                inner_product++;
            }
        }
        
        if (inner_product >= k_snps) {
            resulting_guids_list.append(py::make_tuple(i, inner_product));
        }
    }
    return resulting_guids_list;
}
...
PYBIND11_MODULE(_ardal, m) {  // _ardal module and method bindings
    py::class_<_ardal::AlleleMatrix>(m, "AlleleMatrix")
        ...
        .def("hamming", &_ardal::AlleleMatrix::hamming, py::arg("fill_cache") = false)
        .def("hammingSIMD", &_ardal::AlleleMatrix::hammingSIMD, py::arg("fill_cache") = false)
        ...
        .def("findSharedSNPGUIDs", &_ardal::AlleleMatrix::findSharedSNPGUIDs, py::arg("row_coord"), py::arg("k_snps"))
        .def("findSharedSNPGUIDsSIMD", &_ardal::AlleleMatrix::findSharedSNPGUIDsSIMD, py::arg("row_coord"), py::arg("k_snps"))
